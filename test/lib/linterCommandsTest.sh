...
PreCommitCommandTest() {
  ...
  # shellcheck disable=SC2034
  EXPECTED_LINTER_COMMANDS_ARRAY_PRE_COMMIT=("${BASE_LINTER_COMMANDS_ARRAY_PRE_COMMIT_WITHOUT_FALLBACK_OPTION[@]}" "${PRE_COMMIT_RUN_ALL_FILES_OPTION[@]}")
  # Source the file again so it accounts for modifications
  # shellcheck source=/dev/null
  source "lib/functions/linterCommands.sh"
  if ! AssertArraysElementsContentMatch "LINTER_COMMANDS_ARRAY_PRE_COMMIT" "EXPECTED_LINTER_COMMANDS_ARRAY_PRE_COMMIT"; then
    fatal "LINTER_COMMANDS_ARRAY_PRE_COMMIT doesn't match the expected value when VALIDATE_ALL_CODEBASE is ${VALIDATE_ALL_CODEBASE:-"not initialized"} and GITHUB_BEFORE_SHA is ${GITHUB_BEFORE_SHA:-"not initialized"}"
  fi
  ...
  source "lib/functions/linterCommands.sh"
  if ! AssertArraysElementsContentMatch "LINTER_COMMANDS_ARRAY_PRE_COMMIT" "EXPECTED_LINTER_COMMANDS_ARRAY_PRE_COMMIT"; then
    fatal "LINTER_COMMANDS_ARRAY_PRE_COMMIT doesn't match the expected value when VALIDATE_ALL_CODEBASE is ${VALIDATE_ALL_CODEBASE:-"not set"} and GITHUB_BEFORE_SHA is ${GITHUB_BEFORE_SHA:-"not set"}"
  fi
