...
COPY scripts/create-node-wrappers.sh scripts/linterVersions.sh /
RUN /create-node-wrappers.sh \
  && rm -rfv /create-node-wrappers.sh \
  && /linterVersions.sh \
  && rm -rfv /linterVersions.sh \
  && rm -rfv "${HOME}/.npm" \
  && rm -rfv /node_modules /tmp/global-deps
