...
| **SSH_KEY**                        | `none`                          | SSH key to add to the SSH agent that is created if this variable is provided |
| **SSH_SETUP_GITHUB**               | `false`                         | If set to `true`, adds the `github.com` SSH key to `known_hosts`. This happens automatically if `SSH_KEY` is provided - i.e. this is basically `true` if `SSH_KEY` is provided |
| **SSH_INSECURE_NO_VERIFY_GITHUB_KEY** | `false`                      | If set to `true`, does not verify the fingerprint of the github.com SSH key before adding this. This is not recommended! |
