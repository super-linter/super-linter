...
if [[ ! -x "/node_modules/.bin/${cmd}" ]]; then
  (
    flock 200
    if [[ ! -x "/node_modules/.bin/${cmd}" ]]; then
      mkdir -p /tmp/global-deps
      if [[ ! -f /tmp/global-deps/package.json ]]; then
        echo "{}" > /tmp/global-deps/package.json
      fi
      npm install --prefix /tmp/global-deps --no-audit --no-fund --loglevel=error ${deps[*]} >/dev/null 2>&1
      if [[ ! -e /node_modules ]]; then
        ln -s /tmp/global-deps/node_modules /node_modules
      fi
    fi
  ) 200>/tmp/npm-install.lock
fi
exec "/node_modules/.bin/${cmd}" "\${@}"
EOF
  chmod +x "${target}"
}
