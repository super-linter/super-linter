...
PRETTIER_COMMAND=("${NPX_PRETTIER_COMMAND[@]}")
...
LINTER_COMMANDS_ARRAY_BIOME_FORMAT=("${NPX_BIOME_COMMAND[@]}" format --error-on-warnings --no-errors-on-unmatched)
LINTER_COMMANDS_ARRAY_BIOME_LINT=("${NPX_BIOME_COMMAND[@]}" lint --error-on-warnings --no-errors-on-unmatched)
...
LINTER_COMMANDS_ARRAY_COFFEESCRIPT=("${NPX_COFFEESCRIPT_COMMAND[@]}" -f "${COFFEESCRIPT_LINTER_RULES}")
LINTER_COMMANDS_ARRAY_CPP=(cpplint)
LINTER_COMMANDS_ARRAY_CSHARP=("${DOTNET_FORMAT_COMMAND[@]}" whitespace --folder --exclude / --include "{/}")
LINTER_COMMANDS_ARRAY_CSS=("${NPX_STYLELINT_COMMAND[@]}" --config "${CSS_LINTER_RULES}")
...
LINTER_COMMANDS_ARRAY_GIT_COMMITLINT=("${NPX_COMMITLINT_COMMAND[@]}" --verbose)
...
LINTER_COMMANDS_ARRAY_GROOVY=("${NPX_GROOVY_COMMAND[@]}" -c "${GROOVY_LINTER_RULES}" --failon "${GROOVY_FAILON_LEVEL}" --loglevel "${GROOVY_LOG_LEVEL}" --no-insight)
LINTER_COMMANDS_ARRAY_HTML=("${NPX_HTML_COMMAND[@]}" --config "${HTML_LINTER_RULES}")
...
LINTER_COMMANDS_ARRAY_JAVASCRIPT_ES=("${NPX_ESLINT_COMMAND[@]}" -c "${JAVASCRIPT_ES_LINTER_RULES}")
LINTER_COMMANDS_ARRAY_JAVASCRIPT_PRETTIER=("${PRETTIER_COMMAND[@]}")
LINTER_COMMANDS_ARRAY_JSCPD=("${NPX_JSCPD_COMMAND[@]}" --config "${JSCPD_LINTER_RULES}")
...
LINTER_COMMANDS_ARRAY_JSON=("${NPX_ESLINT_COMMAND[@]}" -c "${JAVASCRIPT_ES_LINTER_RULES}")
LINTER_COMMANDS_ARRAY_JSON_PRETTIER=("${PRETTIER_COMMAND[@]}")
LINTER_COMMANDS_ARRAY_JSONC=("${NPX_ESLINT_COMMAND[@]}" -c "${JAVASCRIPT_ES_LINTER_RULES}")
LINTER_COMMANDS_ARRAY_JSONC_PRETTIER=("${PRETTIER_COMMAND[@]}")
LINTER_COMMANDS_ARRAY_JSX=("${NPX_ESLINT_COMMAND[@]}" -c "${JSX_LINTER_RULES}")
...
LINTER_COMMANDS_ARRAY_MARKDOWN=("${NPX_MARKDOWN_COMMAND[@]}" -c "${MARKDOWN_LINTER_RULES}")
if [ -n "${MARKDOWN_CUSTOM_RULE_GLOBS:-}" ]; then
  ...
  if ! MARKDOWN_LINTER_RULES_DIR_PATH="$(dirname "${MARKDOWN_LINTER_RULES}")"; then
    fatal "Error while initializing ${MARKDOWN_LINTER_RULES_DIR_PATH}"
  fi
  for glob in "${MARKDOWN_CUSTOM_RULE_GLOBS_ARRAY[@]}"; do
    LINTER_COMMANDS_ARRAY_MARKDOWN+=(-r "${MARKDOWN_LINTER_RULES_DIR_PATH}/${glob}")
  done
fi
LINTER_COMMANDS_ARRAY_MARKDOWN_PRETTIER=("${PRETTIER_COMMAND[@]}")
LINTER_COMMANDS_ARRAY_NATURAL_LANGUAGE=("${NPX_TEXTLINT_COMMAND[@]}" -c "${NATURAL_LANGUAGE_LINTER_RULES}")
LINTER_COMMANDS_ARRAY_OPENAPI=("${NPX_OPENAPI_COMMAND[@]}" lint -r "${OPENAPI_LINTER_RULES}" -D)
...
LINTER_COMMANDS_ARRAY_RENOVATE=("${NPX_RENOVATE_COMMAND[@]}" --strict)
...
LINTER_COMMANDS_ARRAY_STATES=("${NPX_STATES_COMMAND[@]}" --json-path)
...
LINTER_COMMANDS_ARRAY_TSX=("${NPX_ESLINT_COMMAND[@]}" -c "${TSX_LINTER_RULES}")
LINTER_COMMANDS_ARRAY_TYPESCRIPT_ES=("${NPX_ESLINT_COMMAND[@]}" -c "${TYPESCRIPT_ES_LINTER_RULES}")
LINTER_COMMANDS_ARRAY_TYPESCRIPT_PRETTIER=("${PRETTIER_COMMAND[@]}")
LINTER_COMMANDS_ARRAY_VUE=("${NPX_ESLINT_COMMAND[@]}" -c "${JAVASCRIPT_ES_LINTER_RULES}")
