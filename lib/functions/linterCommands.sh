...
# Lint all files if:
# - User asked for that (VALIDATE_ALL_CODEBASE=true)
# - VALIDATE_ALL_CODEBASE is set to an empty value (fallback option)
# - GITHUB_BEFORE_SHA is not set (fallback option when linting the initial commit)
if [[ "${VALIDATE_ALL_CODEBASE:-"true"}" == "true" ]] ||
  [[ -z "${GITHUB_BEFORE_SHA:-}" ]]; then
  LINTER_COMMANDS_ARRAY_PRE_COMMIT+=("${PRE_COMMIT_RUN_ALL_FILES_OPTION[@]}")
else
...
LINTER_COMMANDS_ARRAY_PRE_COMMIT+=("${PRE_COMMIT_FROM_FILES_OPTION}" "{}/**")
...
# Add nbQA commands after defining PYTHON commands so we can reuse them
# The format is: nbqa <code quality tool> <notebook or directory> <nbqa options> <code quality tool arguments>
# Reuse the commands we defined above for Python tools to avoid duplication
LINTER_COMMANDS_ARRAY_JUPYTER_NBQA_BLACK=("${NBQA_EXECUTABLE_NAME[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_BLACK[0]}" "{}" "${NBQA_SHELL_OPTION[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_BLACK[@]:1}")
LINTER_COMMANDS_ARRAY_JUPYTER_NBQA_FLAKE8=("${NBQA_EXECUTABLE_NAME[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_FLAKE8[0]}" "{}" "${NBQA_SHELL_OPTION[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_FLAKE8[@]:1}")
LINTER_COMMANDS_ARRAY_JUPYTER_NBQA_ISORT=("${NBQA_EXECUTABLE_NAME[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_ISORT[0]}" "{}" "${NBQA_SHELL_OPTION[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_ISORT[@]:1}")
LINTER_COMMANDS_ARRAY_JUPYTER_NBQA_MYPY=("${NBQA_EXECUTABLE_NAME[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_MYPY[0]}" "{}" "${NBQA_SHELL_OPTION[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_MYPY[@]:1}")
LINTER_COMMANDS_ARRAY_JUPYTER_NBQA_PYLINT=("${NBQA_EXECUTABLE_NAME[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_PYLINT[0]}" "{}" "${NBQA_SHELL_OPTION[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_PYLINT[@]:1}")
# Strip the "check" subcommand because nbqa adds it already
LINTER_COMMANDS_ARRAY_JUPYTER_NBQA_RUFF=("${NBQA_EXECUTABLE_NAME[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_RUFF[0]}" "{}" "${NBQA_SHELL_OPTION[@]}" "${LINTER_COMMANDS_ARRAY_PYTHON_RUFF[@]:2}")
