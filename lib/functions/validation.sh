...
InitializeGitBeforeShaReference() {
  ...
  local GITHUB_EVENT_PUSH_BEFORE="${5}"
  ...
  if [[ "${GITHUB_SHA}" == "${GIT_ROOT_COMMIT_SHA}" ]]; then
    ...
    return 0
  elif [[ "${GITHUB_EVENT_PUSH_BEFORE}" == "0000000000000000000000000000000000000000" ]]; then
    debug "GITHUB_EVENT_PUSH_BEFORE is ${GITHUB_EVENT_PUSH_BEFORE}. Skip initializing GITHUB_BEFORE_SHA"
    return 0
  fi
  debug "${GITHUB_SHA} is not the initial commit, nor all zeroes. Initializing and validating GITHUB_BEFORE_SHA for a ${GITHUB_EVENT_NAME} event"
