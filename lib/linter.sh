...
# shellcheck disable=SC2034  # Variable is referenced indirectly
CLANG_FILE_NAME="${CLANG_CONFIG_FILE:-.clang-format}"
# shellcheck disable=SC2034  # Variable is referenced indirectly
...
# Language array #
##################
LANGUAGE_ARRAY=('ANSIBLE' 'ARM' 'BASH' 'BASH_EXEC' 'CLANG' 'CLOUDFORMATION'
  'CLOJURE' 'COFFEESCRIPT' 'CPP' 'CSHARP' 'CSS' 'DART' 'DOCKERFILE'
  'DOCKERFILE_HADOLINT' 'EDITORCONFIG' 'ENV' 'GHERKIN' 'GO' 'GROOVY' 'HTML'
  'JAVA' 'JAVASCRIPT_ES' "${JAVASCRIPT_STYLE_NAME}" 'JSCPD' 'JSON' 'JSONC'
  'JSX' 'KUBERNETES_KUBEVAL' 'KOTLIN' 'LATEX' 'LUA' 'MARKDOWN'
...
LINTER_NAMES_ARRAY['CLANG']="clang-format"
...
LINTER_COMMANDS_ARRAY['CLANG']="clang-format --Werror --dry-run --style ${CLANG_LINTER_RULES}"
